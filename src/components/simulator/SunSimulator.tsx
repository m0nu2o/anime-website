...
const DEFAULT_PARAMS: SunParams = {
  radius: 120,
  fusion: 2.5,
  convect: 1.2,
  magnetic: 1.4,
...
function ParticleSwarm({ params }: { params: SunParams }) {
  const meshRef = useRef<THREE.InstancedMesh>(null);
  const count = params.particleCount || 15000;
...
export default function SunSimulator({ params = DEFAULT_PARAMS }: { params?: SunParams }) {
  return (
    <div style={{ width: "100%", height: "100%", background: "#050507", position: "relative" }}>
      <Canvas
        camera={{ position: [0, 0, 110], fov: 60 }}
        gl={{ antialias: false, powerPreference: "high-performance" }}
      >
        <fog attach="fog" args={["#050507", 0.01]} />
