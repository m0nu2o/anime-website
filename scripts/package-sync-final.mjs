#!/usr/bin/env node
/**
 * Master Sync Final Production ZIP packaging for NextGen Anime platform.
 *
 * Creates `nextgen-anime-sync-final.zip` at the project root while strictly preserving
 * all existing baseline archives (nextgen-anime-final.zip, nextgen-anime-ultra-final.zip,
 * nextgen-anime-master-final.zip, nextgen-anime-perfect-final.zip) and excluding:
...
const OUTPUT = join(ROOT, "nextgen-anime-sync-final.zip");
...
console.log("[package-sync-final] Scanning workspace for production packaging...");
const files = walk(ROOT);
console.log(`[package-sync-final] Found ${files.length} clean source files to bundle.`);
...
console.log(`[package-sync-final] SUCCESS: ${OUTPUT} generated (${(outStat.size / (1024 * 1024)).toFixed(2)} MB).`);

// Verify contents
const verify = new AdmZip(OUTPUT);
const zipEntries = verify.getEntries().map((e) => e.entryName);
console.log(`[package-sync-final] ZIP verification: ${zipEntries.length} entries bundled.`);
...
const criticalFiles = [
  ...
  "src/app/api/anime/stream/route.ts",
...
for (const c of criticalFiles) {
  ...
  if (!found) {
    console.error(`[package-sync-final] CRITICAL WARNING: Missing file in ZIP: ${c}`);
    process.exit(1);
  }
}
console.log("[package-sync-final] All critical production files verified in ZIP archive.");
